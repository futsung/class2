...
class GameManager:
    def __init__(self):
        ...
        self.match_sound: pygame.mixer.Sound = self._load_sound("music/caraccident.mp3")
    ...
    def show_start_screen(self) -> None:
        ...
        start_background = pygame.transform.scale(
            start_background, (self.screen_width, self.screen_height)
        )
    ...
    def main_loop(self) -> None:
        ...
        while self.running:
            ...
            HealthBar.draw(
                ...
                enemy_x,
                enemy_y - 20,
                self.enemy_sizes[self.current_enemy_index][0],
                10,
            )
        ...
        while waiting:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    pygame.quit()
                    exit()
                if event.type == pygame.MOUSEBUTTONDOWN and button_rect.collidepoint(
                    event.pos
                ):
                    waiting = False

    @staticmethod
    def show_summary(screen: pygame.Surface, traffic_tickets: int, images: Dict[str, pygame.Surface]) -> None:
        screen.fill((0, 0, 0))
        screen.blit(images["victory"], (0, 0))

        font = pygame.font.Font(None, 72)
        text_number = font.render(f"{traffic_tickets}", True, (255, 255, 255))
        number_rect = text_number.get_rect(
