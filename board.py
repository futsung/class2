...
class TileManager:
    ...
    @staticmethod
    def draw_tiles(tiles: List[List[Optional[Runestone]]], screen: pygame.Surface, images: dict, tile_size: int) -> None:
        """
        繪製符石與棋盤格線。
        """
        for row in range(len(tiles)):
    ...
    @staticmethod
    def check_matches(tiles: List[List[Optional[Runestone]]]) -> set:
        """
        檢查盤面上的三消匹配，並移除匹配的格子。
        """
        matched = set()
        for row in range(len(tiles)):
            for col in range(len(tiles[0]) - 2):
                if (
                    ...
                    and tiles[row][col + 1]
                    and tiles[row][col + 2]
                    and tiles[row][col].type == tiles[row][col + 1].type == tiles[row][col + 2].type
                ):
        ...
        for col in range(len(tiles[0])):
            for row in range(len(tiles) - 2):
                if (
                    ...
                    and tiles[row + 1][col]
                    and tiles[row + 2][col]
                    and tiles[row][col].type == tiles[row + 1][col].type == tiles[row + 2][col].type
                ):
                    matched.update({(row, col), (row + 1, col), (row + 2)})
